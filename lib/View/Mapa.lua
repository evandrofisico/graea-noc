...
function Mapa:ConvertData(mapa)
local n_mapa = {}
for uf,data in pairs(mapa) do
	n_mapa[uf] = {}
	for index,pontos in pairs(data) do
		n_mapa[uf][index] = {}
		n_mapa[uf][index].x = (pontos.x)*self.properties.scale+self.properties.border.x
		n_mapa[uf][index].y = (907 - pontos.y)*self.properties.scale+self.properties.border.y

	end
end
self.properties.mapa = n_mapa
end

function Mapa:InsertLocations(data)
self.properties["location"] = data
end


function Mapa:ConvertPoint(lat,long)
local vt = { lat = 33.7486, long = 73.7983}
local d = { lat = math.abs(lat), long = math.abs(long)}
local delta_x = vt.long - d.long
local delta_y = vt.lat - d.lat

local conv_f = 23.25
local pixels_x = d.long*conv_f
local pixels_y = d.lat*conv_f

local cx =  (1720-pixels_x)*self.properties.scale+self.properties.border.x
local cy =  (125+pixels_y)*self.properties.scale+self.properties.border.y
return cx, cy
end

--function Mapa:ConvertPoint(nlat,nlong)
--local zoom = 4.9
--local lat = (math.pi*nlat)/180
--local long = (math.pi*nlong)/180
--
--local x = (128/math.pi)*math.pow(2,zoom)*(long+math.pi)
--local y = (128/math.pi)*math.pow(2,zoom)*(math.pi-math.log( math.tan( math.pi/4 + lat/2 )))
---- usando projecao webmercator
----vt = { lat = 33.7486, long = 73.7983}
----d = { lat = math.abs(lat), long = math.abs(long)}
----delta_x = vt.long - d.long
----delta_y = vt.lat - d.lat
----
----conv_f = 23.25
----pixels_x = d.long*conv_f
----pixels_y = d.lat*conv_f
--
----local cx =  (1720-x)*self.properties.scale+self.properties.border.x
----local cy =  (125+y)*self.properties.scale+self.properties.border.y
--x = x - 2500
--y = y - 3900
--print(x,y)
--return x, y
...
local px,py = self:ConvertPoint(lat,long)	
...
local ua_name = string.gsub(unit,"_"," ")
