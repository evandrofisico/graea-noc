...
$returndata = array();  //json data agregated
...
switch($type){
	...
	case 'source_type':
		...
		else 
		{
		    while (!$rg->EOF) 
		    {
				...
				$txt_pt[$rg->fields["id"]] = $rg->fields["name"];
	...
	case 'category':	
		...
		else
		{
		    while (!$rg->EOF) 
		    {
			    ...
			    $returndata[$rg->fields["name"]] = $rg->fields["num_events"];
	...
	case 'alarmstickets':
		...
		$label = array(_('Today'),_('-1 Day'),_('-2 Days'),_('Week'),_('2 Weeks'));
